...
describe('kana decks', () => {
  ...
  it('cover 46 base + 25 voiced/semi-voiced + 33 yōon = 104 cards per script', () => {
    ...
    expect(hira.settings).toEqual({ baseTimerMs: 5000, newCardsPerSession: 5, maxBox1ForNew: 10, retrievalAllowanceMs: 3500 });
  });
...
describe('piano decks', () => {
  ...
  it('cover the DESIGN ranges: treble C4–C6 (15), bass C2–E4 (17), all with SVG media', () => {
    ...
    expect(treble.deck.settings).toEqual({ baseTimerMs: 7000, newCardsPerSession: 5, maxBox1ForNew: 10, retrievalAllowanceMs: 2200 });
  });
